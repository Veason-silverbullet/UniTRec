...
from textRec_datasets.newsrec_dataset import NewsRecTrainDataset, NewsRecValDataset
...
def parse_config():
    parser = argparse.ArgumentParser(description='NewsRec')
    ...
    parser.add_argument('--decoder_seq_len', type=int, default=64, help='Decoder sequence length')
...
def train(args):
    ...
    train_dataset = NewsRecTrainDataset(args)
    train_dataset.negative_sampling(epoch=1)
    if args.local_rank == -1:
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
        val_dataset = NewsRecValDataset(args, mode='dev')
        test_dataset = NewsRecValDataset(args, mode='test')
    else:
        train_sampler = DistributedSampler(train_dataset, shuffle=True)
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, sampler=train_sampler)
        val_dataset = NewsRecValDataset(args, mode='dev', rank=args.local_rank, world_size=dist.get_world_size())
        test_dataset = NewsRecValDataset(args, mode='test', rank=args.local_rank, world_size=dist.get_world_size())
    ...
    if args.local_rank == -1:
        if args.task == 'newsRec/MIND-small':
            auc, mrr, ndcg5, ndcg10, hr1, hr5, hr10 = inference(args, val_model, test_dataset, result_file, return_scores=True)
            print(Fore.BLUE + ('MIND-small\nAUC = %.2f\nMRR = %.2f\nnDCG@5 = %.2f\nnDCG@10 = %.2f\nHR@1 = %.2f\nHR@5 = %.2f\nHR@10 = %.2f' % (auc, mrr, ndcg5, ndcg10, hr1, hr5, hr10)) + Fore.RESET)
        else:
            inference(args, val_model, test_dataset, result_file, return_scores=False)
            os.chdir(args.prediction_dir)
            with zipfile.ZipFile('prediction.zip', 'w') as f:
                f.write('prediction.txt')
    else: # distributed inference and aggregation results
...
def test(args):
    ...
    if args.local_rank == -1:
        val_dataset = NewsRecValDataset(args, mode='test')
        ...
        val_dataset = NewsRecValDataset(args, mode='test', rank=args.local_rank, world_size=dist.get_world_size())
