...
from textRec_datasets.engagerec_dataset import EngageRecTrainDataset, EngageRecValDataset
...
def parse_config():
    parser = argparse.ArgumentParser(description='EngageRec')
    ...
    parser.add_argument('--encoder_seq_len', type=int, default=1024, help='Encoder sequence length')
    parser.add_argument('--decoder_seq_len', type=int, default=576, help='Decoder sequence length')
    ...
    parser.add_argument('--negative_sample_num', type=int, default=14, help='Number of negative samples')
    ...
    parser.add_argument('--task', type=str, default='engageRec/Reddit-engage_technology', choices=['engageRec/Reddit-engage_technology', 'engageRec/Reddit-engage_funny', 'engageRec/Reddit-engage_todayilearned'], help='Text-based recommendation tasks')
...
def train(args):
    ...
    train_dataset = EngageRecTrainDataset(args)
    train_dataset.negative_sampling(epoch=1)
    if args.local_rank == -1:
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
        val_dataset = EngageRecValDataset(args, mode='dev')
        test_dataset = EngageRecValDataset(args, mode='test')
    else:
        train_sampler = DistributedSampler(train_dataset, shuffle=True)
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, sampler=train_sampler)
        val_dataset = EngageRecValDataset(args, mode='dev', rank=args.local_rank, world_size=dist.get_world_size())
        test_dataset = EngageRecValDataset(args, mode='test', rank=args.local_rank, world_size=dist.get_world_size())
                ...
                if args.local_rank == -1:
                    ...
                    if args.local_rank == 0:
                        ...
                        with open(val_dataset.truth_file, 'r', encoding='utf-8') as truth_f, open(result_file, 'r', encoding='utf-8') as result_f:
                            auc, mrr, ndcg5, ndcg10, hr1, hr5, hr10 = scoring(truth_f, result_f)
...
def inference(args, model, val_dataset, result_file, return_scores):
    ...
    assert len(val_dataloader) % ((val_dataset.candidate_num - 1) // val_dataset.VAL_ENGAGE_SEGMENT_NUM + 1) == 0
    val_num = len(val_dataloader) // ((val_dataset.candidate_num - 1) // val_dataset.VAL_ENGAGE_SEGMENT_NUM + 1)
...
def test(args):
    ...
    if args.local_rank == -1:
        val_dataset = EngageRecValDataset(args, mode='test')
        ...
        val_dataset = EngageRecValDataset(args, mode='test', rank=args.local_rank, world_size=dist.get_world_size())
